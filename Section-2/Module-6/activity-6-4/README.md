...
This activity focuses on using the $\Sigma$ matrix to limit the principal components based on how much variance should be kept. In Codio Activity 6.3, you used a scree plot to observe when the difference in variance explained slows. Here, you will determine how many components are required to explain a proportion of variance. The dataset is a larger example of a housing dataset related to individual houses and features in Ames, Iowa. For our purposes, the non-null numeric data is selected.
...
### How do I open the activity?
 To view the activity, click on the file `coding_activity_6.4.ipynb` in the panel on the left.
