...
This activity focuses on using the DBSCAN clustering algorithm with sklearn. Contrary to the k-means clustering function, DBSCAN does not take a number of clusters as an argument. Instead, the algorithm separates data based on areas of high and low density. The parameters that determine this density are the min_samples and eps values. We will explore how different values for these affect the clustering for our sample dataset.
...
### How do I open the activity?
 To view the activity, click on the file `coding_activity_6.8.ipynb` in the panel on the left.
